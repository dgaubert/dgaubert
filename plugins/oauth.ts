...
const { signIn: signInGoogle, handleCallback: handleCallbackGoogle } =
  createHelpers(
    createGoogleOAuthConfig({
...
const { signIn: signInFacebook, handleCallback: handleCallbackFacebook } =
  createHelpers(
    createFacebookOAuthConfig({
...
export default {
  ...
  routes: [
    ...
    },
    {
      ...
      async handler(req) {
        const { response, tokens, sessionId } = await handleCallbackFacebook(
          req,
        );
...
async function getFacebookUser(accessToken: string): Promise<FacebookUser> {
  const response = await fetch(
    `https://graph.facebook.com/v19.0/me?fields=email&access_token=${accessToken}`,
  );
