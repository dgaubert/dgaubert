...
import { Media, getPost } from "@/utils/posts.ts";
...
export const handler: Handlers<PageData, RequestState> = {
  async GET(_req, ctx) {
    try {
      ...
      return ctx.render({ post: post as Media });
    } catch {
      return ctx.renderNotFound();
    }
  },
...
export default function PostPage(props: PageProps<PageData, RequestState>) {
  const post = props.data.post as Media;
  ...
  return (
    <>
      ...
      <main class="max-w-screen-sm mx-auto px-6 pt-16 pb-4">
        ...
        />
        <iframe
          class="mt-12 w-full aspect-video"
          src={post.link} 
